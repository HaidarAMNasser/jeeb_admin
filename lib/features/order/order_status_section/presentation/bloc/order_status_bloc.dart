...
class OrderStatusBloc extends Bloc<OrderStatusEvent, OrderStatusState> {
  OrderStatusBloc({
    ...
    String? initialStatusWire,
    double? deliveryLatitude,
    double? deliveryLongitude,
  })  : _orderStatusRtdb = orderStatusRtdb,
        _lastStatusWire = _normalizeWire(initialStatusWire) ??
            initialStatus.apiWireValue,
        super(
  ...
  void _startRealtimeListener() {
    ...
    _rtdbSub?.cancel();
    _rtdbSub = _orderStatusRtdb.watchOrderStatusWire(state.orderId).listen(
          (wire) {
            if (isClosed) return;
    ...
    _deliveryIdSub =
        _orderStatusRtdb.watchOrderDeliveryId(state.orderId).listen(
              (deliveryId) {
                if (isClosed) return;
