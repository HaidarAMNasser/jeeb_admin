...
import 'package:jeeb_admin/features/merchant/reset_merchant_password/data/data_sources/reset_merchant_password_data_source.dart';

class ResetMerchantPasswordRepository {
  final ResetMerchantPasswordRemoteDataSource _remoteDataSource;
  final NetworkInfo _networkInfo;

  const ResetMerchantPasswordRepository(
    this._remoteDataSource,
    this._networkInfo,
  );

  Future<Either<Failure, void>> resetMerchantPassword({
    required String id,
    required String password,
  }) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await _remoteDataSource.resetMerchantPassword(
          id: id,
          password: password,
        );
